...
from utils.sample import cochran_sample_size
import sqlite3
import time
import re
import numpy as np
...
def search_example_by_question(question, example_encoder_questions, example_decoder_answers,limit_table,nshot):
    ...
    for i in top_indexes:
        ...
        user = [{"role": "user", "content": f"{db_str}\n{query}"}, {"role": "assistant", "content": 'Sketch: '+template+'\nVQL:'+g}]
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,engine,data_type,model_type,limit_table, nshot):
    ...
    for i, question in enumerate(pbar):
        ...
        pre_text = 'You are a data visualization assistant. Gnerate VQL in one single line and begin with : visualize. Let\'s think step by step. Generate the sketch as intermediate representation and then the final VQL.'
        ...
        if count % 300 == 0:
            save_data(generated_answers, f'error_total_{model_type}_{data_type}_create_select_data{limit_table}_{nshot}shot_{count}_sketch_role.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'error_total_{model_type}_{data_type}_create_select_data{limit_table}_{nshot}shot_total_sketch_role.txt'
    print(save_pre_file)
