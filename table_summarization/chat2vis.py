...
def get_schema(db, table_names, model,question,limit):
    ...
    for table_name in table_names:
        ...
        for row in rows:
            ...
            for x in row:
                if isinstance(x, str):
                    row_data.append(x.lower())
                else:
                    row_data.append(x)
            table_data.append(row_data)
        conn.close()

        description = f"Use table {table_name} with columns {', '.join(column_names)}. "

        def get_column_type(table_data, column_name):

            distinct_values = set()
            index = table_data[0].index(column_name)

            for row in table_data[1:]:
                value = row[index]
                distinct_values.add(value)

            type = get_value_data_type(distinct_values)

            if type == 'object' and len(distinct_values) < 20:
                values = ", ".join(str(value) for value in distinct_values)
                return f"has categorical values: {values}. "
            elif type == 'object':
                return f'is type {type} and contains string values. '
            elif type == 'int64' or type == 'float64':
                return f'is type {type} and contains numeric values. '

        def get_value_data_type(values):
            data_types = set()
            for value in values:
        ...
        for column in column_names:
            description += f"The column {column} {get_column_type(table_data, column)}"
        tables.append(description)

    return ''.join(tables)

def search_example_by_question(question, example_encoder_questions, example_decoder_answers,model,limit_table,nshot):
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table,nshot):
    ...
    for i, question in enumerate(pbar):
        ...
        example_data = search_example_by_question(question, example_encoder_questions, example_decoder_answers,model,limit_table,nshot)
        ...
        if count % 300 == 0:
            save_data(generated_answers, f'{model_type}_{data_type}_chat2vis_{nshot}shot_{count}.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'{model_type}_{data_type}_chat2vis_data_{nshot}shot_total.txt'


    test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table,nshot)
