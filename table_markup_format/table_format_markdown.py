...
def get_schema(db, table_names, model,question,limit):
    ...
    for table_name in table_names:
        ...
        tables.append(data)
    markdown = json_to_markdown(tables)
    # print(markdown)
    conn.close()
    return markdown
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table):
    ...
    for i, question in enumerate(pbar):
        ...
        if count % 100 == 0:
            save_data(generated_answers, f'{model_type}_{data_type}_markdown_data{limit_table}_oneshot_{count}.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'{model_type}_{data_type}_markdown_data{limit_table}_oneshot_total.txt'
