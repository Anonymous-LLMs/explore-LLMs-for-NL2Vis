...
def json_to_xml(data):
    ...
    for table in parsed_data:
        # 将表名作为XML元素
        table_element = ET.SubElement(root, table['TableName'])
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table):
    ...
    for i, question in enumerate(pbar):
        ...
        if count % 300 == 0:
            save_data(generated_answers, f'{model_type}_{data_type}_xml_data{limit_table}_oneshot_{count}.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'{model_type}_{data_type}_xml_data{limit_table}_oneshot_total.txt'
