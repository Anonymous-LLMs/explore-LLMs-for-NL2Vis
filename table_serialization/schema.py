...
def get_schema(db, table_names, model,question,limit):
    ...
    for table_name in table_names:
        ...
        for description in cursor.description:
            column_names = column_names + ' ' + str(description[0]).lower()
        ...
        for _, row in top_rows:
            ...
            for x in row:
                table_data = table_data + ' ' + str(x).lower()
        data = data + ' ' + table_name + " " + column_names + " " + table_data
        # data = {
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table):
    ...
    for i, question in enumerate(pbar):
        ...
        if count % 300 == 0:
            save_data(generated_answers, f'{model_type}_{data_type}_serial_schema_data{limit_table}_oneshot_{count}.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'{model_type}_{data_type}_serial_schema_data{limit_table}_oneshot_total.txt'
