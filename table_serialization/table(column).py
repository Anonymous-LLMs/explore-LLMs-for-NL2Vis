...
def get_schema(db, table_names, model, question, limit,fk):
    ...
    for table_name in table_names:
        ...
        tables = tables + table_sample + foreign_keys
    tables = '### Postgres SQL tables, with their properties:\n#'+tables+'#'
    conn.close()
    return tables


def search_example_by_question(question, example_encoder_questions, example_decoder_answers,model,limit_table,nshot,fk):
...
def test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table,nshot,fk):
    ...
    for i, question in enumerate(pbar):
        ...
        example_data = search_example_by_question(question, example_encoder_questions, example_decoder_answers,model,limit_table,nshot,fk)
        ...
        if count % 300 == 0:
            save_data(generated_answers, f'{model_type}_{data_type}_serial_api_{nshot}shot_{count}.txt')
...
if __name__ == "__main__":
    ...
    save_pre_file = f'{model_type}_{data_type}_serial_api_fk{fk}_{nshot}shot_total.txt'


    test(train_encoder_file,train_decoder_file,test_encoder_file,test_decoder_file,save_pre_file,model,data_type,model_type,limit_table,nshot,fk)
